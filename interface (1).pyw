...
def geometry_params(active_state):
    ...
    states = {
        ...
        LINES: (
            'Количество лучей:', NUM_RAYS,
            'Длина луча:', RAY_LENGTH,
        ),
        PARABOLAS: (
            'Параметр c (точка перехода):', C_PARAM,
            'Параметр C (конец линейной части):', C_END,
            'Начальное n:', N_START,
            'Конечное n:', N_END,
            'Шаг n:', N_STEP,
        ),
    ...
    num_pairs = len(state) // 2

    for i in range(num_pairs):
        label_text = state[i * 2]
        entry_name = state[i * 2 + 1]

        label = ttk.Label(frame, text=label_text)
        label.grid(column=0, row=i * 2, sticky='nsew')

        entry = ttk.Entry(frame, name=entry_name)
        entry.grid(column=0, row=i * 2 + 1, sticky='nsew', pady=(0, 4))
...
def start(n, zeta0, kappa0, combobox_geom, frame, notebook, progress_value):
    ...
    elif combobox_geom == LINES:
        num_rays = int(frame.nametowidget(NUM_RAYS).get())
        ray_length = float(frame.nametowidget(RAY_LENGTH).get())
        cm.line(numRays=num_rays, rayLength=mp.mpf(ray_length))
        add_to_note(notebook, combobox_geom,
                    cm.plot_gradient(cm.zetaMatrix, draw_arrows=False, labels={'X': 'Real', 'Y': 'Imag'}))
    elif combobox_geom == PARABOLAS:
        c = float(frame.nametowidget(C_PARAM).get())
        C_end = float(frame.nametowidget(C_END).get())
        n_start = int(frame.nametowidget(N_START).get())
        n_end = int(frame.nametowidget(N_END).get())
        n_step = int(frame.nametowidget(N_STEP).get())
        cm.parabolas(c, C_end, n_start, n_end, n_step)  # вызов обновлённого метода
        add_to_note(notebook, combobox_geom,
                    cm.plot_gradient(cm.zetaMatrix, draw_arrows=False, labels={'X': 'Real', 'Y': 'Imag'}))
    #cm.calcKappa()
...
param_canvas = tkinter.Canvas(geometryframe, highlightthickness=0)
param_scroll = ttk.Scrollbar(geometryframe, orient='vertical', command=param_canvas.yview)
param_canvas.configure(yscrollcommand=param_scroll.set)

param_canvas.grid(column=0, row=2, sticky='nsew')
param_scroll.grid(column=1, row=2, sticky='ns')

# Внутренний фрейм, в котором будут размещаться поля ввода
paramframe = ttk.Frame(param_canvas)
param_canvas.create_window((0, 0), window=paramframe, anchor='nw', tags='inner')

# Автоматическое обновление области прокрутки
def _configure_inner(event):
    param_canvas.configure(scrollregion=param_canvas.bbox('all'))
paramframe.bind('<Configure>', _configure_inner)

# Чтобы холст корректно менял размер
geometryframe.columnconfigure(0, weight=1)
geometryframe.columnconfigure(1, weight=0)
geometryframe.rowconfigure(2, weight=1)
